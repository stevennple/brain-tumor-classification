...
# Provides a user-friendly explanation of the saliency map for an MRI scan, focusing on the key areas highlighted by the model.
...
# Generates a saliency map by computing gradients of the predicted class with respect to input pixels.
...
# Generates a Grad-CAM visualization to highlight regions most relevant for the prediction.
...
# Loads and returns the Xception-based model with specified weights. Raises a FileNotFoundError if weights are missing.
@st.cache_resource
